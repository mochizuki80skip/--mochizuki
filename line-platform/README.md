...
> 接骨院・サロン等の予約システムとして、LIFF カレンダー UI でお客様が予約 → スプレッドシートに自動記録、までを完結できます。スプレッドシートには予約一覧・スケジュール（空き状況マトリクス）・各種設定が自動同期されます。

## 主要機能

- LINE 公式アカウント運用（マルチテナント / メンバー管理）
- 友だち / タグ / 一斉配信 / ステップ配信
- 予約機能（LIFF カレンダー + スプレッドシート連携）
  - 予約タブ：予約データの履歴
  - スケジュールタブ：日 × 時間のマトリクスで空き状況を可視化
  - 設定 / メニュー / 営業時間 / きっかけタブ：自動同期
