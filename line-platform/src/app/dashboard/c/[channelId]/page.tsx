...
export default async function ChannelDashboard({ params }: { params: Promise<{ channelId: string }> }) {
  const { channelId } = await params;

  const [followers, totalFriends, tags, scheduledBroadcasts, runningScenarios, recentInbound] =
    await Promise.all([
      prisma.friend.count({ where: { lineChannelId: channelId, isFollowing: true } }),
      prisma.friend.count({ where: { lineChannelId: channelId } }),
      prisma.tag.count({ where: { lineChannelId: channelId } }),
      prisma.broadcast.count({ where: { lineChannelId: channelId, status: "scheduled" } }),
      prisma.scenarioRun.count({
        where: { status: "running", friend: { lineChannelId: channelId } },
      }),
      prisma.inboundMessage.findMany({
        where: { friend: { lineChannelId: channelId } },
        take: 10,
